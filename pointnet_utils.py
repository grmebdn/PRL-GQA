...
class PointNetEncoder(nn.Module):
    ...
    def forward(self, x):  # 输入B x D X N
        ...
        x = torch.max(x, 2, keepdim=True)[0]    # B x D X N
...
if __name__ == '__main__':
    data = np.random.randint(1,10,size=(3,3,2))
    print(data)
    ...
    for batch in range(B):
        data3[batch]=data[batch]-data2[batch]
    m = np.max(np.sqrt(np.sum(data3 ** 2, axis=-1)), axis=-1)
    print(data3)
    print(data3[0].shape)
    print(m.shape)
    print(m[0])
